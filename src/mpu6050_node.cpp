...
#define AVG_BUF_SIZE 400 //shoulbe less than time_offset
...
MPU6050Node::MPU6050Node() {
    ...
    /* setting publishers */
    data_pub = nh.advertise<sensor_msgs::Imu>("imu/data", 50);
    euler_pub = nh.advertise<geometry_msgs::Vector3Stamped>("imu/euler", 50);
...
bool MPU6050Node::wait_for_warmup() {
    ...
    if((now-begin-warmup_time).toSec() > 0) { warmup_done = true; }
